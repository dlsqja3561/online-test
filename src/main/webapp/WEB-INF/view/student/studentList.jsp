	...
	<!-- empMenu include -->
	 ...
	 <div>
	 	...
	 	<c:if test="${currentPage <= 1}">
	 		<a href="${pageContext.request.contextPath}/employee/student/studentList?currentPage=1&searchWord=${searchWord}">이전</a>
	 	</c:if>
	 	<!-- 이전페이지 -->
	 	<c:if test="${currentPage > 1}">
	 	...
	 	<c:forEach var="s" begin="${startPage}" end="${endPage}" step="1">
	 		<a href="${pageContext.request.contextPath}/employee/student/studentList?currentPage=${s}&searchWord=${searchWord}">${s}</a>
	 	</c:forEach>
	 	<!-- 다음페이지 -->
	 	...
	 	<!-- 마지막페이지에서 다음버튼클릭시 -->
	 	<c:if test="${currentPage >= lastPage}">
	 		<a href="${pageContext.request.contextPath}/employee/student/studentList?currentPage=${lastPage}&searchWord=${searchWord}">다음</a>
	 	</c:if>
	 	<!-- 마지막으로 -->
